...
const eventDeleteService_1 = require("./src/services/eventDeleteService");
const orphanService_1 = require("./src/services/orphanService");
const mergeSuggestionService_1 = require("./src/services/mergeSuggestionService");
...
const whatifService_1 = require("./src/services/whatifService");
const weekResultsService_1 = require("./src/services/weekResultsService");
const teamService_1 = require("./src/services/teamService");
const skinsService_1 = require("./src/services/skinsService");
const removePlayerService_1 = require("./src/services/removePlayerService");
const randomTeamsService_1 = require("./src/services/randomTeamsService");
const teamGameService_1 = require("./src/services/teamGameService");
const playingGroupService_1 = require("./src/services/playingGroupService");
...
const teeTimesLinkService_1 = require("./src/services/teeTimesLinkService");
...
const calendarService_1 = require("./src/services/calendarService");
const courseHistoryService_1 = require("./src/services/courseHistoryService");
...
app.get('/api/master-events', async (req, res) => {
    try {
        const query = req.query.q || '';
        const events = await (0, eventService_1.searchAllEventsForMaster)(query);
        res.json(events);
    }
    catch (error) {
...
app.get('/api/events/cleanup-candidates-all', async (req, res) => {
    try {
        const candidates = await (0, cleanupService_1.getAllEventsCleanupCandidates)();
        res.json(candidates);
    }
    catch (error) {
...
app.get('/api/events/all-with-counts', async (req, res) => {
    try {
        const events = await (0, eventDeleteService_1.getAllEventsWithCounts)();
        res.json(events);
    }
    catch (error) {
...
app.delete('/api/events/:id', async (req, res) => {
    try {
        ...
        await (0, eventDeleteService_1.deleteEvent)(eventId);
...
app.get('/api/events/orphaned', async (req, res) => {
    try {
        const links = await (0, playerMergeService_1.getOrphanedEventLinks)();
        res.json(links);
    }
    catch (error) {
...
app.post('/api/events/orphaned/delete', async (req, res) => {
    try {
        ...
        await (0, playerMergeService_1.deleteOrphanedEventLinks)(eventIds.map(Number));
        res.json({ success: true });
    }
    catch (error) {
...
app.get('/api/orphaned-rows', async (req, res) => {
    try {
        const rows = await (0, orphanService_1.getOrphanedRows)();
        res.json(rows);
    }
    catch (error) {
...
app.post('/api/orphaned-rows/delete', async (req, res) => {
    try {
        ...
        await (0, orphanService_1.deleteOrphanedRows)(keys);
        res.json({ success: true });
    }
    catch (error) {
...
app.get('/api/merge-suggestions', async (req, res) => {
    try {
        const suggestions = await (0, mergeSuggestionService_1.getMergeSuggestions)();
...
app.post('/api/merge-suggestions/ignore', async (req, res) => {
    try {
        ...
        await (0, mergeSuggestionService_1.ignoreMergeSuggestion)(playerId1, playerId2);
        res.json({ success: true });
    }
    catch (error) {
...
app.patch('/api/events/:id', async (req, res) => {
    try {
        ...
        if (eventName !== undefined) {
            const result = await (0, eventService_1.renameEvent)(id, eventName);
            if (!result.ok)
                return res.status(400).json({ error: result.error });
        }
        if (courseName !== undefined) {
            const result = await (0, eventService_1.renameEventCourse)(id, courseName);
            if (!result.ok)
                return res.status(400).json({ error: result.error });
        }
        res.json({ status: 'ok' });
...
app.post('/api/courses', async (req, res) => {
    try {
        ...
        const courseId = await (0, courseService_1.createCourse)(courseName, holes, ghinInfo);
        if (Array.isArray(ghinTeeSets) && ghinTeeSets.length > 0) {
            await (0, courseService_1.saveCourseTeeSets)(courseId, ghinTeeSets);
        }
...
app.get('/api/courses/:id/history', async (req, res) => {
    try {
        ...
        const rows = await (0, courseHistoryService_1.getCourseHoleHistory)(courseId);
        res.json(rows);
    }
    catch (error) {
...
app.get('/api/events/:id/teetimes-players', async (req, res) => {
    try {
       ...
       ORDER BY p.LastName, p.FirstName`, [eventIds]);
        res.json(rows);
    }
    catch (error) {
...
app.post('/api/events/:id/players', async (req, res) => {
    try {
        ...
        await (0, playerService_1.addPlayer)(eventId, { firstName, lastName, email: email || '', phone: phone || '' });
        res.json({ success: true });
    }
    catch (error) {
...
app.post('/api/events/:id/assign-guest', async (req, res) => {
    try {
        ...
        const guest = await (0, playerService_1.assignGuestPlayer)(eventId, Number(courseId), Array.isArray(excludePlayerIds) ? excludePlayerIds.map(Number) : []);
        res.json(guest);
    }
    catch (error) {
...
app.get('/api/events/:id/unlinked-players', async (req, res) => {
    try {
        const eventId = parseInt(req.params.id);
        const rows = await (0, playerService_1.getUnlinkedPlayers)(eventId);
        res.json(rows);
    }
    catch (error) {
...
app.post('/api/events/:id/link-players', async (req, res) => {
    try {
        ...
        await (0, playerService_1.linkPlayers)(eventId, playerIds);
        res.json({ success: true });
    }
    catch (error) {
...
app.get('/api/events/:id/player-list', async (req, res) => {
    try {
        const eventId = parseInt(req.params.id);
        const rows = await (0, playerService_1.getPlayerListForEvent)(eventId);
        res.json(rows);
    }
    catch (error) {
...
app.patch('/api/players/:id', async (req, res) => {
    try {
        ...
        const result = await (0, playerService_1.renamePlayer)(playerId, firstName, lastName);
        if (!result.ok) {
...
app.get('/api/players/search', async (req, res) => {
    try {
        const query = req.query.q || '';
        const players = await (0, playerMergeService_1.searchAllPlayers)(query);
        res.json(players);
    }
    catch (error) {
...
app.get('/api/events/:id/players/search', async (req, res) => {
    try {
        const eventId = parseInt(req.params.id);
        const query = req.query.q || '';
        const players = await (0, playerMergeService_1.searchPlayersInEvent)(eventId, query);
        res.json(players);
    }
    catch (error) {
...
app.get('/api/players/merge-preview', async (req, res) => {
    try {
        const duplicateId = parseInt(req.query.duplicateId);
        ...
        const preview = await (0, playerMergeService_1.getMergePreview)(duplicateId, targetId);
        res.json(preview);
    }
    catch (error) {
...
app.post('/api/players/merge', async (req, res) => {
    try {
        ...
        const result = await (0, playerMergeService_1.mergePlayers)(Number(duplicateId), Number(targetId));
        if (!result.ok) {
            return res.status(400).json({ error: result.error });
        }
        res.json({ success: true });
...
app.get('/api/players/orphaned', async (req, res) => {
    try {
        const players = await (0, playerMergeService_1.getOrphanedPlayers)();
        res.json(players);
    }
    catch (error) {
...
app.get('/api/players/invalid-names', async (req, res) => {
    try {
        const players = await (0, playerMergeService_1.getInvalidNamePlayers)();
        res.json(players);
    }
    catch (error) {
...
app.post('/api/players/orphaned/delete', async (req, res) => {
    try {
        ...
        await (0, playerMergeService_1.deleteOrphanedPlayers)(playerIds.map(Number));
        res.json({ success: true });
    }
    catch (error) {
...
app.get('/api/games/orphaned', async (req, res) => {
    try {
        const games = await (0, playerMergeService_1.getEmptyGames)();
        res.json(games);
    }
    catch (error) {
...
app.post('/api/games/orphaned/delete', async (req, res) => {
    try {
        ...
        await (0, playerMergeService_1.deleteEmptyGames)(gameIds.map(Number));
        res.json({ success: true });
    }
    catch (error) {
...
app.get('/api/players/history-list', async (req, res) => {
    try {
        const players = await (0, playerHistoryService_1.getAllPlayersWithHistory)();
        res.json(players);
    }
    catch (error) {
...
app.get('/api/players/:id/skins-stats', async (req, res) => {
    try {
        const playerId = parseInt(req.params.id);
        const stats = await (0, skinsHistoryService_1.getPlayerSkinsStats)(playerId);
        res.json(stats);
    }
    catch (error) {
...
app.get('/api/players/:id/recent-skins', async (req, res) => {
    try {
        const playerId = parseInt(req.params.id);
        const rounds = await (0, skinsHistoryService_1.getPlayerRecentSkins)(playerId);
        res.json(rounds);
    }
    catch (error) {
...
app.get('/api/players/:id/stats', async (req, res) => {
    try {
        const playerId = parseInt(req.params.id);
        const stats = await (0, playerHistoryService_1.getPlayerStats)(playerId);
        res.json(stats);
    }
    catch (error) {
...
app.get('/api/players/:id/rounds', async (req, res) => {
    try {
        const playerId = parseInt(req.params.id);
        const rounds = await (0, playerHistoryService_1.getPlayerRounds)(playerId);
        res.json(rounds);
    }
    catch (error) {
...
app.get('/api/events/:id/teetimes', async (req, res) => {
    try {
       ...
       ORDER BY TeeDate`, [eventIds]);
        const byDate = new Map();
        for (const r of rows) {
            const key = String(r.TeeDate);
            if (!byDate.has(key) || r.GroupID === eventId)
                byDate.set(key, r);
        }
...
app.get('/api/events/:id/paid-tracker', async (req, res) => {
    try {
        ...
        const rows = await (0, paidTrackerService_1.getPaidTrackerList)(eventId, teeDate);
        res.json(rows);
    }
    catch (error) {
...
app.post('/api/events/:id/paid-tracker', async (req, res) => {
    try {
        ...
        await (0, paidTrackerService_1.setPaidTracker)(eventId, teeDate, playerId, paid);
        res.json({ success: true });
    }
    catch (error) {
...
app.post('/api/events/:id/teetimes', async (req, res) => {
    try {
        ...
        await (0, teetimesService_1.addTeeTime)(eventId, { teeDate, time1, time2, time3, time4, time5 });
        res.json({ success: true });
    }
    catch (error) {
...
app.get('/api/games/:id/naughty-list', async (req, res) => {
    try {
        ...
        const rows = await (0, ghinService_1.getNaughtyList)(gameId);
        res.json(rows);
    }
    catch (error) {
...
app.post('/api/games/:id/players/:playerId/naughty-recheck', async (req, res) => {
    try {
        ...
        const result = await (0, ghinService_1.recheckLatePosting)(gameId, playerId);
        res.json(result);
    }
    catch (error) {
...
app.get('/api/events/:id/ghin-players', async (req, res) => {
    try {
        const eventId = parseInt(req.params.id);
        const rows = await (0, ghinService_1.getGhinPlayerList)(eventId);
        res.json(rows);
    }
    catch (error) {
...
app.get('/api/events/:id/ghin-easy-links', async (req, res) => {
    try {
        const eventId = parseInt(req.params.id);
        const candidates = await (0, ghinService_1.findEasyGhinLinks)(eventId);
        res.json(candidates);
    }
    catch (error) {
...
app.get('/api/events/:id/ghin-summary', async (req, res) => {
    try {
        ...
        const rows = await (0, ghinService_1.getGhinSummary)(eventId, year);
        res.json(rows);
    }
    catch (error) {
...
app.get('/api/events/:id/ghin-years', async (req, res) => {
    try {
        const eventId = parseInt(req.params.id);
        const years = await (0, ghinService_1.getGhinYears)(eventId);
        res.json(years);
    }
    catch (error) {
...
app.post('/api/events/:id/options', async (req, res) => {
    try {
        const eventId = parseInt(req.params.id);
        await (0, optionsService_1.saveEventOptions)(eventId, req.body);
        res.json({ success: true });
    }
    catch (error) {
...
app.get('/api/events/:id/admin-password-status', async (req, res) => {
    try {
        const eventId = parseInt(req.params.id);
        const hasPassword = await (0, optionsService_1.hasAdminPassword)(eventId);
...
app.post('/api/events/:id/verify-admin-password', async (req, res) => {
    try {
        ...
        const valid = await (0, optionsService_1.verifyAdminPassword)(eventId, password ?? '');
        res.json({ valid });
    }
    catch (error) {
...
app.post('/api/events/:id/admin-password', async (req, res) => {
    try {
        const eventId = parseInt(req.params.id);
        const { password } = req.body;
        await (0, optionsService_1.setAdminPassword)(eventId, password ?? '');
        res.json({ success: true });
    }
    catch (error) {
...
app.get('/api/events/:id/ups-cup-winners', async (req, res) => {
    try {
        const eventId = parseInt(req.params.id);
        const winners = await (0, upsCupService_1.getUpsCupWinners)(eventId);
        res.json(winners);
    }
    catch (error) {
...
app.post('/api/events/:id/ups-cup-winners', async (req, res) => {
    try {
        ...
        await (0, upsCupService_1.setUpsCupWinner)(eventId, parseInt(year), parseInt(playerId));
        res.json({ success: true });
    }
    catch (error) {
...
app.delete('/api/events/:id/ups-cup-winners/:year', async (req, res) => {
    try {
        ...
        await (0, upsCupService_1.deleteUpsCupWinner)(eventId, year);
        res.json({ success: true });
    }
    catch (error) {
...
app.get('/api/events/:id/major-winners', async (req, res) => {
    try {
        ...
        const winners = await (0, upsCupService_1.getMajorWinners)(eventId, year);
        res.json(winners);
    }
    catch (error) {
...
app.get('/api/games/:id/ups-points', async (req, res) => {
    try {
        const gameId = parseInt(req.params.id);
        const points = await (0, upsCupService_1.getUpsPointsForGame)(gameId);
        res.json(points);
    }
    catch (error) {
...
app.get('/api/events/:id/ineligible-players', async (req, res) => {
    try {
        const eventId = parseInt(req.params.id);
        const year = parseInt(req.query.year) || new Date().getFullYear();
        const ineligible = await (0, upsCupService_1.getIneligiblePlayers)(eventId, year);
        res.json(ineligible);
    }
    catch (error) {
...
app.get('/api/events/:id/ups-standings', async (req, res) => {
    try {
        const eventId = parseInt(req.params.id);
        const year = parseInt(req.query.year) || new Date().getFullYear();
        const standings = await (0, upsCupService_1.getCurrentStandings)(eventId, year);
        res.json(standings);
    }
    catch (error) {
...
app.get('/api/events/:id/ups-paid-players', async (req, res) => {
    try {
        const eventId = parseInt(req.params.id);
        const year = parseInt(req.query.year) || new Date().getFullYear();
        const paid = await (0, upsCupService_1.getPaidPlayers)(eventId, year);
        res.json(paid);
    }
    catch (error) {
...
app.post('/api/events/:id/ups-paid-players', async (req, res) => {
    try {
        ...
        await (0, upsCupService_1.setPlayerPaid)(eventId, parseInt(year), parseInt(playerId));
        res.json({ success: true });
    }
    catch (error) {
...
app.delete('/api/events/:id/ups-paid-players/:year/:playerId', async (req, res) => {
    try {
        ...
        const year = parseInt(req.params.year);
        const playerId = parseInt(req.params.playerId);
        await (0, upsCupService_1.removePlayerPaid)(eventId, year, playerId);
        res.json({ success: true });
    }
    catch (error) {
...
app.get('/api/events/:id/birdie-leaderboard', async (req, res) => {
    try {
        const eventId = parseInt(req.params.id);
        const year = parseInt(req.query.year) || new Date().getFullYear();
        const rows = await (0, birdieRaceService_1.getBirdieLeaderboard)(eventId, year);
        res.json(rows);
    }
    catch (error) {
...
app.get('/api/events/:id/birdie-player-status', async (req, res) => {
    try {
        ...
        const rows = await (0, birdieRaceService_1.getPlayerBirdieStatus)(eventId, year, playerId);
        res.json(rows);
    }
    catch (error) {
...
app.get('/api/events/:id/birdie-hole-detail', async (req, res) => {
    try {
        const eventId = parseInt(req.params.id);
        const year = parseInt(req.query.year) || new Date().getFullYear();
        const hole = parseInt(req.query.hole);
        const rows = await (0, birdieRaceService_1.getHoleBirdieDetail)(eventId, year, hole);
        res.json(rows);
    }
    catch (error) {
...
app.get('/api/events/:id/calendar', async (req, res) => {
    try {
        const eventId = parseInt(req.params.id);
        const year = parseInt(req.query.year) || new Date().getFullYear();
        const days = await (0, calendarService_1.getCalendarForYear)(eventId, year);
        res.json(days);
    }
    catch (error) {
...
app.post('/api/events/:id/calendar', async (req, res) => {
    try {
        ...
        await (0, calendarService_1.setCalendarDay)(eventId, date, dayType, note ?? '');
        res.json({ success: true });
    }
    catch (error) {
...
app.delete('/api/events/:id/calendar/:date', async (req, res) => {
    try {
        ...
        await (0, calendarService_1.deleteCalendarDay)(eventId, date);
        res.json({ success: true });
    }
    catch (error) {
...
app.get('/api/events/:id/cleanup-candidates', async (req, res) => {
    try {
        const eventId = parseInt(req.params.id);
        const candidates = await (0, cleanupService_1.getCleanupCandidates)(eventId);
        res.json(candidates);
    }
    catch (error) {
...
app.post('/api/events/:id/cleanup-candidates/delete', async (req, res) => {
    try {
        ...
        await (0, cleanupService_1.deleteCleanupCandidates)(eventId, items);
        res.json({ success: true });
    }
    catch (error) {
...
app.post('/api/events/:id/cleanup-candidates/ignore', async (req, res) => {
    try {
        ...
        await (0, cleanupService_1.ignoreCleanupCandidates)(eventId, items);
        res.json({ success: true });
    }
    catch (error) {
...
app.get('/api/events/:id/cleanup-ignored', async (req, res) => {
    try {
        const eventId = parseInt(req.params.id);
        const rows = await (0, cleanupService_1.getIgnoredCleanupItems)(eventId);
        res.json(rows);
    }
    catch (error) {
...
app.delete('/api/events/:id/cleanup-ignored/:key', async (req, res) => {
    try {
        ...
        await (0, cleanupService_1.unignoreCleanupItem)(eventId, key);
        res.json({ success: true });
    }
    catch (error) {
...
app.get('/api/events/:id/opted-out', async (req, res) => {
    try {
        const eventId = parseInt(req.params.id);
        const rows = await (0, cleanupService_1.getOptedOutPlayers)(eventId);
        res.json(rows);
    }
    catch (error) {
...
app.get('/api/events/:id/status', async (req, res) => {
    try {
       ...
       ORDER BY ps.LastUpdateDt ASC`, [eventIds, teeDate]);
        const byPlayer = new Map();
        for (const r of rows) {
            const existing = byPlayer.get(r.PlayerID);
            if (!existing || r.GroupID === eventId)
...
app.post('/api/games/:id/handicap', async (req, res) => {
    try {
        ...
        await config_1.default.query(`INSERT INTO Hdcp (GameID, PlayerID, Hdcp, LastUpdateUser)
       VALUES (?, ?, ?, 'app')
       ON DUPLICATE KEY UPDATE Hdcp = VALUES(Hdcp), LastUpdateDt = CURRENT_TIMESTAMP`, [gameId, playerId, hdcp]);
...
app.post('/api/scores/:gameId/:playerId/delete-holes', async (req, res) => {
    try {
        ...
        const success = await (0, scoreService_1.deletePlayerHoleScores)(gameId, playerId, holeNumbers);
        if (success) {
            res.json({ success: true });
        }
        else {
...
app.get('/api/scores/:gameId/:playerId', async (req, res) => {
    try {
        ...
        const scores = await (0, scoreService_1.getPlayerScores)(gameId, playerId);
        res.json(scores);
    }
    catch (error) {
...
app.get('/api/events/:id/team-game-weeks', async (req, res) => {
    try {
        const eventId = parseInt(req.params.id);
        const weeks = await (0, teamGameService_1.getTeamGameWeeks)(eventId);
        res.json(weeks);
    }
    catch (error) {
...
app.post('/api/events/:id/ensure-game', async (req, res) => {
    try {
        ...
        const gameId = await (0, gameService_2.getOrCreateGameForDate)(eventId, date, courseId);
        res.json({ gameId });
    }
    catch (error) {
...
app.post('/api/games/:id/course', async (req, res) => {
    try {
        ...
        if (!courseId)
            return res.status(400).json({ error: 'courseId is required' });
        await (0, gameService_2.updateGameCourse)(gameId, courseId);
        res.json({ success: true });
    }
    catch (error) {
...
app.get('/api/games/:id/default-side', async (req, res) => {
    try {
        const gameId = parseInt(req.params.id);
        const side = await (0, leaderboardService_1.getDefaultScorecardSide)(gameId);
        res.json({ side });
    }
    catch (error) {
...
app.get('/api/games/:id/scorecard', async (req, res) => {
    try {
        ...
        const rows = await (0, leaderboardService_1.getGameScorecard)(gameId, side, scoreType);
        res.json(rows);
    }
    catch (error) {
...
app.get('/api/events/:id/games', async (req, res) => {
    try {
        const eventId = parseInt(req.params.id);
        const games = await (0, whatifService_1.getGamesForEvent)(eventId);
        res.json(games);
    }
    catch (error) {
...
app.get('/api/games/:id/players', async (req, res) => {
    try {
        const gameId = parseInt(req.params.id);
        const players = await (0, whatifService_1.getPlayersForGame)(gameId);
        res.json(players);
    }
    catch (error) {
...
app.get('/api/games/:id/swap-preview', async (req, res) => {
    try {
        const gameId = parseInt(req.params.id);
        const playerAId = parseInt(req.query.playerAId);
        const playerBId = parseInt(req.query.playerBId);
        if (!playerAId || !playerBId) {
            return res.status(400).json({ error: 'playerAId and playerBId query params are required' });
        }
        const preview = await (0, playerMergeService_1.getGameSwapPreview)(gameId, playerAId, playerBId);
        res.json(preview);
    }
    catch (error) {
...
app.post('/api/games/:id/swap', async (req, res) => {
    try {
        ...
        const result = await (0, playerMergeService_1.swapPlayersInGame)(gameId, Number(playerAId), Number(playerBId), handicap);
        if (!result.ok) {
            return res.status(400).json({ error: result.error });
        }
        res.json({ success: true });
...
app.get('/api/games/:id/whatif-team-games', async (req, res) => {
    try {
        const gameId = parseInt(req.params.id);
        const options = await (0, whatifService_1.getWhatIfTeamGameOptions)(gameId);
        res.json(options);
    }
    catch (error) {
...
app.get('/api/games/:id/whatif', async (req, res) => {
    try {
        const gameId = parseInt(req.params.id);
        const playerId = parseInt(req.query.playerId);
        const teamGameId = req.query.teamGameId ? parseInt(req.query.teamGameId) : undefined;
        if (!playerId) {
            return res.status(400).json({ error: 'playerId query param is required' });
        }
        const rows = await (0, whatifService_1.getWhatIfResults)(gameId, playerId, teamGameId);
        res.json(rows);
    }
    catch (error) {
...
app.get('/api/games/:id/results', async (req, res) => {
    try {
        ...
        const rows = await (0, weekResultsService_1.getWeekResults)(gameId, scoreType);
        res.json(rows);
    }
    catch (error) {
...
app.get('/api/games/:id/teams/status', async (req, res) => {
    try {
        const gameId = parseInt(req.params.id);
        const exists = await (0, teamService_1.hasTeams)(gameId);
...
app.get('/api/games/:id/teams', async (req, res) => {
    try {
        const gameId = parseInt(req.params.id);
        const rows = await (0, teamService_1.getTeamResults)(gameId);
        res.json(rows);
    }
    catch (error) {
...
app.get('/api/games/:id/cut-summary', async (req, res) => {
    try {
        const gameId = parseInt(req.params.id);
        const summary = await (0, randomTeamsService_1.getCutSummary)(gameId);
        res.json(summary);
    }
    catch (error) {
...
app.get('/api/games/:id/team-assignments', async (req, res) => {
    try {
        const gameId = parseInt(req.params.id);
        const teams = await (0, teamService_1.getTeamAssignments)(gameId);
        res.json(teams);
    }
    catch (error) {
...
app.post('/api/games/:id/teams', async (req, res) => {
    try {
        ...
        await (0, teamService_1.saveTeams)(gameId, teams);
...
app.get('/api/games/:id/teams/:teamId/scorecard', async (req, res) => {
    try {
        ...
        const rows = await (0, teamService_1.getTeamScorecard)(gameId, teamId, side);
        res.json(rows);
    }
    catch (error) {
...
app.post('/api/games/:id/team-games/skip', async (req, res) => {
    try {
        ...
        const teamGameId = await (0, teamGameService_1.skipTeamGameSlot)(gameId, label, slot);
        res.json({ teamGameId });
    }
    catch (error) {
...
app.delete('/api/team-games/:teamGameId', async (req, res) => {
    try {
        ...
        await (0, teamGameService_1.deleteTeamGame)(teamGameId);
        res.json({ success: true });
    }
    catch (error) {
...
app.get('/api/team-games/:teamGameId/status', async (req, res) => {
    try {
        const teamGameId = parseInt(req.params.teamGameId);
        const status = await (0, teamGameService_1.getTeamGameStatus)(teamGameId);
        if (!status) {
...
app.get('/api/team-games/:teamGameId/assignments', async (req, res) => {
    try {
        const teamGameId = parseInt(req.params.teamGameId);
        const teams = await (0, teamGameService_1.getTeamGameAssignments)(teamGameId);
        res.json(teams);
    }
    catch (error) {
...
app.post('/api/team-games/:teamGameId/teams', async (req, res) => {
    try {
        ...
        await (0, teamGameService_1.saveManualTeamGameTeams)(teamGameId, teams);
        res.json({ message: 'Teams saved successfully' });
    }
    catch (error) {
...
app.get('/api/team-games/:teamGameId/roster', async (req, res) => {
    try {
        const teamGameId = parseInt(req.params.teamGameId);
        const roster = await (0, teamGameService_1.getTeamGameRoster)(teamGameId);
        res.json(roster);
    }
    catch (error) {
...
app.post('/api/team-games/:teamGameId/random', async (req, res) => {
    try {
        const teamGameId = parseInt(req.params.teamGameId);
        const result = await (0, teamGameService_1.createRandomTeamGameTeams)(teamGameId);
        res.json(result);
    }
    catch (error) {
...
app.get('/api/team-games/:teamGameId/results', async (req, res) => {
    try {
        const teamGameId = parseInt(req.params.teamGameId);
        const rows = await (0, teamGameService_1.getTeamGameResults)(teamGameId);
        res.json(rows);
    }
    catch (error) {
...
app.get('/api/team-games/:teamGameId/cut-summary', async (req, res) => {
    try {
        const teamGameId = parseInt(req.params.teamGameId);
        const summary = await (0, teamGameService_1.getTeamGameCutSummary)(teamGameId);
        res.json(summary);
    }
    catch (error) {
...
app.get('/api/team-games/:teamGameId/scorecard/:teamNumber', async (req, res) => {
    try {
        ...
        const rows = await (0, teamGameService_1.getTeamGameScorecard)(teamGameId, teamNumber, side);
        res.json(rows);
    }
    catch (error) {
...
app.post('/api/games/:id/group-team', async (req, res) => {
    try {
        ...
        const result = await (0, teamGameService_1.addOrUpdateGroupTeam)(gameId, playerIds);
        res.json(result);
    }
    catch (error) {
...
app.post('/api/games/:id/playing-group', async (req, res) => {
    try {
        ...
        await (0, playingGroupService_1.upsertPlayingGroup)(gameId, playerIds);
        res.json({ success: true });
    }
    catch (error) {
...
app.get('/api/games/:id/playing-group/:playerId', async (req, res) => {
    try {
        const gameId = parseInt(req.params.id);
        const playerId = parseInt(req.params.playerId);
        const teammateIds = await (0, playingGroupService_1.getPlayingGroup)(gameId, playerId);
...
app.get('/api/games/:id/skins', async (req, res) => {
    try {
        ...
        if (hole === 'T') {
            const totals = await (0, skinsService_1.getSkinsTotals)(gameId);
            return res.json(totals);
        }
        const holeId = parseInt(hole);
        const result = await (0, skinsService_1.getSkinsForHole)(gameId, holeId);
        res.json(result);
    }
    catch (error) {
...
app.get('/api/games/:id/skins-summary', async (req, res) => {
    try {
        const gameId = parseInt(req.params.id);
        const rows = await (0, skinsService_1.getGameSkinsSummary)(gameId);
        res.json(rows);
    }
    catch (error) {
...
app.get('/api/games/:id/active-players', async (req, res) => {
    try {
        const gameId = parseInt(req.params.id);
        const players = await (0, removePlayerService_1.getActivePlayersForGame)(gameId);
        res.json(players);
    }
    catch (error) {
...
app.post('/api/games/:id/optout', async (req, res) => {
    try {
        ...
        await (0, removePlayerService_1.removePlayers)(gameId, playerIds);
...
app.get('/api/events/:id/random-teams-status', async (req, res) => {
    try {
        const eventId = parseInt(req.params.id);
        const info = await (0, randomTeamsService_1.getLatestGameForRandomTeams)(eventId);
        if (!info) {
            return res.status(404).json({ error: 'No games found for this event' });
        }
        res.json(info);
...
app.get('/api/games/:id/show-teams', async (req, res) => {
    try {
        const gameId = parseInt(req.params.id);
        const teams = await (0, teamGameService_1.getShowTeamsListing)(gameId);
        res.json(teams);
    }
    catch (error) {
...
app.get('/api/games/:id/random-teams', async (req, res) => {
    try {
        const gameId = parseInt(req.params.id);
        const teams = await (0, randomTeamsService_1.getRandomTeamsListing)(gameId);
        res.json(teams);
    }
    catch (error) {
...
app.post('/api/games/:id/random-teams', async (req, res) => {
    try {
        const gameId = parseInt(req.params.id);
        const result = await (0, randomTeamsService_1.createRandomTeams)(gameId);
        res.json(result);
    }
    catch (error) {
...
app.post('/api/feedback', async (req, res) => {
    try {
        ...
        await (0, feedbackService_1.submitFeedback)(type, String(title).trim(), description || '', submittedBy || '');
        res.json({ success: true });
    }
    catch (error) {
...
app.get('/api/feedback', async (req, res) => {
    try {
        const entries = await (0, feedbackService_1.getFeedback)();
        res.json(entries);
    }
    catch (error) {
...
app.delete('/api/feedback/:id', async (req, res) => {
    try {
        ...
        await (0, feedbackService_1.deleteFeedback)(feedbackId);
        res.json({ success: true });
    }
    catch (error) {
