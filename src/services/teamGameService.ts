...
function isKeepFormat(format: string): boolean {
  return format === TEAMS_KEEP_FORMAT || format === '36' || format === '48';
}
...
function isIrishFormat(format: string): boolean {
  return format === IRISH_RUMBLE_FORMAT;
}
import type { TeamResultRow, TeamScorecardRow } from './teamService';
import type { RandomTeamPlayer, RandomTeamResult, CreateRandomTeamsResult } from './randomTeamsService';

export type TeamGameAssignMode = 'R' | 'M' | 'G';

/** Which Options "Teams N" card (1-4) a given slot number maps to. */
function slotPrefix(slot: number): 'teams' | 'teams2' | 'teams3' | 'teams4' {
  const capped = Math.min(4, Math.max(1, slot));
  return capped === 1 ? 'teams' : (`teams${capped}` as 'teams2' | 'teams3' | 'teams4');
}

export interface TeamGameWeek {
  date: string; // 'YYYY-MM-DD'
  // Null for a scheduled future date with no Game yet — Team Games shows the pure Options
  // preview for these (see app/teamgames.tsx) and only creates a real Game the moment an admin
  // actually acts on it (getOrCreateGameForDate in gameService.ts).
  gameId: number | null;
  courseId: number;
  courseName: string;
}
 ...
 */
export async function getTeamGameWeeks(eventId: number): Promise<TeamGameWeek[]> {
  const [gameRows] = await pool.query<any[]>(
    `SELECT DISTINCT g.GameID, g.GameDate, g.CourseID, c.CourseName
  ...
  const [defaultCourseRows] = await pool.query<any[]>(
    `SELECT g.CourseID, c.CourseName FROM Game g INNER JOIN Course c ON c.CourseID = g.CourseID
     WHERE g.GroupID = ? ORDER BY g.GameDate DESC LIMIT 1`,
    [eventId]
  );
  const defaultCourseId = defaultCourseRows[0]?.CourseID ?? 0;
  const defaultCourseName = defaultCourseRows[0]?.CourseName ?? '';

  const byDate = new Map<string, TeamGameWeek>();
...
/** A one-off team game's own payout/cut config, stored directly on its TeamGame row -- same
 * shape as an Options "Teams N" card's teams_netcut/netcut9/payin/places/pct1-4 fields, but
 * settable independently per one-off game instead of being read from Options (see createTeamGame's
 * doc comment). All optional/blank means "no cut" / "no payout" for that field. */
...
export interface TeamGameSummary {
  ...
  randomSet: boolean;
  lastHoleAll: boolean;
  // Payout/cut config stored directly on this row -- see TeamGamePayoutInput. Null for any field
  // never set (including every team game created before this existed).
  netCut: number | null;
  netCut9: number | null;
  payIn: number | null;
  places: number | null;
  ...
  pct3: number | null;
  pct4: number | null;
  // A placeholder row created by "Skip this week" on a pending Options slot — has no players and
  // can't be scored/assigned. Deleting it (the same ✕ as any other team game) undoes the skip and
  // brings back the "Set Up Teams N" prompt for that slot.
  skipped: boolean;
  // True once any player has been assigned to a team (Randomize run, Manual saved, or a Playing
  // Groups auto-fill) — once true, this team game can no longer be deleted (see deleteTeamGame),
  // so the UI hides the ✕ rather than offering a button that's guaranteed to be refused.
  hasPlayers: boolean;
  // Which Options "Teams N" card (1-4) this row belongs to, explicitly recorded at creation time
  // — NOT inferred from creation order, since any slot can be set up or skipped independently of
  // the others (confirmed with the user 2026-07-08: Teams 1 and Teams 2 are both freely
  // actionable regardless of order). Null for a one-off team game not tied to any Options card.
  slot: number | null;
  // 'custom' (the classic keep-best-X-of-N-every-hole rule, KeepCount above), '36/48' (a team
  // that picks live, hole by hole, how many net scores to keep, targeting its own actual
  // headcount x 12 -- see TeamGameHoleKeep), or 'irish' (Irish Rumble: a fixed, deterministic
 ...
 */
export async function listTeamGames(gameId: number): Promise<TeamGameSummary[]> {
  const [rows] = await pool.query<any[]>(
    `SELECT tg.*, (SELECT COUNT(*) FROM TeamGamePlayer tgp WHERE tgp.TeamGameID = tg.TeamGameID) AS PlayerCount
     FROM TeamGame tg WHERE tg.GameID = ?
     ORDER BY (tg.Slot IS NULL), tg.Slot, tg.TeamGameID`,
    [gameId]
  );
  return rows.map((r) => ({
...
export async function createTeamGame(
  ...
  if (![2, 3, 4].includes(teamSize)) throw new Error('teamSize must be 2, 3, or 4');
  if (keepCount < 1 || keepCount > teamSize) throw new Error('keepCount must be between 1 and teamSize');
  if (assignMode === 'R' && teamSize !== 2) throw new Error('Random assignment is only available for 2-person teams');
 ...
 */
export async function skipTeamGameSlot(gameId: number, label: string, slot: number): Promise<number> {
  const [result] = await pool.query<any>(
    `INSERT INTO TeamGame (GameID, Label, TeamSize, KeepCount, AssignMode, Skipped, Slot, LastUpdateUser)
     VALUES (?, ?, 2, 1, 'M', 'T', ?, 'App')`,
    [gameId, label, slot]
  );
  return result.insertId;
 ...
 */
export async function deleteTeamGame(teamGameId: number): Promise<void> {
  const [rows] = await pool.query<any[]>(
    'SELECT COUNT(*) AS cnt FROM TeamGamePlayer WHERE TeamGameID = ?',
    [teamGameId]
  );
  if (Number(rows[0].cnt) > 0) {
    throw new Error('This team game has already been drawn and can no longer be deleted.');
  }
  await pool.query('DELETE FROM TeamGamePlayer WHERE TeamGameID = ?', [teamGameId]);
  await pool.query('DELETE FROM TeamGame WHERE TeamGameID = ?', [teamGameId]);
}
...
export async function getTeamGameStatus(teamGameId: number): Promise<TeamGameStatus | null> {
  const [rows] = await pool.query<any[]>('SELECT * FROM TeamGame WHERE TeamGameID = ?', [teamGameId]);
  if (rows.length === 0) return null;
  const tg = rows[0];
  const { stillPlaying } = await getGameEligibility(tg.GameID);
  return {
 ...
 */
export async function getTeamGameAssignments(teamGameId: number): Promise<number[][]> {
  const [rows] = await pool.query<any[]>(
    'SELECT TeamNumber, PlayerID FROM TeamGamePlayer WHERE TeamGameID = ? ORDER BY TeamNumber, PlayerID',
    [teamGameId]
  );
 ...
 */
export async function saveManualTeamGameTeams(teamGameId: number, teams: number[][]): Promise<void> {
  await pool.query('DELETE FROM TeamGamePlayer WHERE TeamGameID = ?', [teamGameId]);
  ...
  await pool.query(
    `INSERT INTO TeamGamePlayer (TeamGameID, TeamNumber, PlayerID, LastUpdateUser) VALUES ${rowPlaceholders}`,
    params
  );
}
 ...
 */
export async function getTeamGameRoster(teamGameId: number): Promise<RandomTeamResult[]> {
  const [teamRows] = await pool.query<any[]>(
    `SELECT tgp.TeamNumber, tgp.PlayerID, CONCAT(p.LastName, ', ', p.FirstName) AS name, g.GameID, g.GameDate
     ...
     ORDER BY tgp.TeamNumber, p.LastName, p.FirstName`,
    [teamGameId]
  );
  ...
  const [hdcpRows] = await pool.query<any[]>(
    `SELECT PlayerID, GameID, Hdcp, LastUpdateDt FROM Hdcp WHERE PlayerID IN (?) ORDER BY LastUpdateDt ASC`,
    [playerIds]
  );

  const historyByPlayer = new Map<number, { gameId: number; hdcp: number; date: Date }[]>();
  ...
  function resolveHdcp(playerId: number, gameDate: Date): number | null {
    const history = historyByPlayer.get(playerId);
    if (!history || history.length === 0) return null;

    const exact = history.find((h) => h.gameId === gameId);
    if (exact) return exact.hdcp;

    const priorOrSame = history.filter((h) => h.date <= gameDate);
    if (priorOrSame.length > 0) return priorOrSame[priorOrSame.length - 1].hdcp;

    return history[0].hdcp;
  }

  const byTeam = new Map<number, RandomTeamResult>();
  for (const r of teamRows) {
    if (!byTeam.has(r.TeamNumber)) byTeam.set(r.TeamNumber, { teamId: r.TeamNumber, players: [] });
    byTeam.get(r.TeamNumber)!.players.push({ name: r.name, hdcp: resolveHdcp(r.PlayerID, r.GameDate) });
  }

  for (const team of byTeam.values()) {
    team.players.sort((a: RandomTeamPlayer, b: RandomTeamPlayer) => (a.hdcp ?? Infinity) - (b.hdcp ?? Infinity) || a.name.localeCompare(b.name));
  }

  return Array.from(byTeam.values()).sort((a, b) => a.teamId - b.teamId);
}

/** One team game's roster, labeled — for a game with more than one active "Teams N" card,
 * so a read-only listing can group them instead of mixing team numbers from different cards
 * together under one ambiguous "Team 1", "Team 2"... */
 ...
 */
export async function getShowTeamsListing(gameId: number): Promise<LabeledTeamGameResult[]> {
  const teamGames = (await listTeamGames(gameId)).filter((tg) => tg.hasPlayers);

  if (teamGames.length > 0) {
    const multiple = teamGames.length > 1;
    const results: LabeledTeamGameResult[] = [];
  ...
  const legacy = await getRandomTeamsListing(gameId);
 ...
 */
export async function createRandomTeamGameTeams(teamGameId: number): Promise<CreateRandomTeamsResult> {
  const [tgRows] = await pool.query<any[]>(
    'SELECT GameID, AssignMode, RandomSet, Slot, NetCut, NetCut9 FROM TeamGame WHERE TeamGameID = ?',
    [teamGameId]
  );
  if (tgRows.length === 0) throw new Error('Team game not found');
  const tg = tgRows[0];
  if (tg.AssignMode !== 'R') throw new Error('This team game is not set to random assignment');
  ...
  const eligibility = tg.Slot
    ? await getGameEligibility(tg.GameID, slotPrefix(tg.Slot))
    : await getGameEligibility(tg.GameID, 'none', {
  ...
  const [playerRows] = await pool.query<any[]>(
    `SELECT PlayerID FROM Player WHERE PlayerID IN (?) ORDER BY RAND()`,
    [eligibility.eligiblePlayerIds.length > 0 ? eligibility.eligiblePlayerIds : [0]]
  );
  ...
  const teamRows: { teamNumber: number; playerId: number }[] = [];

  for (const row of playerRows) {
    if (posInTeam === PLAYERS_PER_TEAM) posInTeam = 0;
    posInTeam++;
    if (firstPlayerId === null) firstPlayerId = row.PlayerID;
    if (posInTeam === 1) teamNumber++;
  ...
  await pool.query(`UPDATE TeamGame SET RandomSet = 'T' WHERE TeamGameID = ?`, [teamGameId]);
 ...
 */
async function autoProvisionPartnerTeamsSlots(gameId: number): Promise<void> {
  const [gameRows] = await pool.query<any[]>('SELECT GroupID FROM Game WHERE GameID = ?', [gameId]);
  ...
  const [existingSlotRows] = await pool.query<any[]>(
    'SELECT Slot FROM TeamGame WHERE GameID = ? AND Slot IS NOT NULL',
    [gameId]
  );
  ...
  for (let slot = 1; slot <= totalConfiguredSlots; slot++) {
    ...
    if (!options[`${prefix}_partnerteams`]) continue;

    const format = (options[`${prefix}_format`] as string) || 'custom';
    ...
    await pool.query(
      `INSERT INTO TeamGame (GameID, Label, TeamSize, KeepCount, AssignMode, LastHoleAll, Slot, Format, LastUpdateUser)
       VALUES (?, ?, ?, ?, 'G', ?, ?, ?, 'App')`,
      [gameId, `Teams ${slot}`, teamSize, keepCount, lastHoleAll ? 'T' : 'F', slot, format]
    );
  }
}
 ...
 */
export async function addOrUpdateGroupTeam(gameId: number, playerIds: number[]): Promise<{ teamGameIds: number[] }> {
  if (playerIds.length < 2) return { teamGameIds: [] };

  await autoProvisionPartnerTeamsSlots(gameId);

  const [groupTeamGames] = await pool.query<any[]>(
    `SELECT TeamGameID FROM TeamGame WHERE GameID = ? AND AssignMode = 'G'`,
    [gameId]
  );
  if (groupTeamGames.length === 0) return { teamGameIds: [] };
  ...
  for (const tg of groupTeamGames) {
    ...
    const [existingRows] = await pool.query<any[]>(
      'SELECT TeamNumber, PlayerID FROM TeamGamePlayer WHERE TeamGameID = ?',
      [teamGameId]
    );
    ...
    await pool.query('DELETE FROM TeamGamePlayer WHERE TeamGameID = ? AND PlayerID IN (?)', [teamGameId, playerIds]);
...
function hashSeed(teamGameId: number, teamNumber: number, holeId: number): number {
...
async function getMaxRosterSize(teamGameId: number): Promise<number> {
  const [rows] = await pool.query<any[]>(
    'SELECT TeamNumber, COUNT(*) AS cnt FROM TeamGamePlayer WHERE TeamGameID = ? GROUP BY TeamNumber',
    [teamGameId]
  );
 ...
 */
export async function getTeamGameResults(teamGameId: number): Promise<TeamResultRow[]> {
  const [tgRows] = await pool.query<any[]>(
    `SELECT tg.GameID, tg.KeepCount, tg.LastHoleAll, tg.Format FROM TeamGame tg WHERE tg.TeamGameID = ?`,
    [teamGameId]
  );
  ...
  const format = (tgRows[0].Format as string) || 'custom';
  ...
  if (showParRelative) {
    const [gameCourseRows] = await pool.query<any[]>('SELECT CourseID FROM Game WHERE GameID = ?', [gameId]);
    const courseId = gameCourseRows[0]?.CourseID;
    const [holeRows] = await pool.query<any[]>('SELECT HoleNum, Par FROM CourseDetails WHERE CourseID = ?', [courseId]);
    parByHole = new Map(holeRows.map((h) => [h.HoleNum, h.Par]));
  }

  const [rows] = await pool.query<any[]>(
    `SELECT tgp.TeamNumber, tgp.PlayerID, s.HoleID, s.NetScore
     FROM TeamGamePlayer tgp
  ...
  const holeScores = new Map<number, Map<number, { playerId: number; net: number }[]>>();
  for (const r of rows) {
    if (!holeScores.has(r.TeamNumber)) holeScores.set(r.TeamNumber, new Map());
    const perHole = holeScores.get(r.TeamNumber)!;
    if (!perHole.has(r.HoleID)) perHole.set(r.HoleID, []);
  ...
  const holeKeepByTeam = new Map<number, Map<number, number>>();
  if (usesLiveKeepPicker) {
    const [keepRows] = await pool.query<any[]>(
      'SELECT TeamNumber, HoleID, KeepCount FROM TeamGameHoleKeep WHERE TeamGameID = ?',
      [teamGameId]
    );
    for (const r of keepRows) {
      if (!holeKeepByTeam.has(r.TeamNumber)) holeKeepByTeam.set(r.TeamNumber, new Map());
  ...
  function sumSide(
    teamNumber: number,
    perHole: Map<number, { playerId: number; net: number }[]>,
    holeFilter: (h: number) => boolean
  ): { net: number; par: number } {
    const lastHole = lastHoleAll ? 18 : null;
    let net = 0;
    let par = 0;
    for (const [holeId, present] of perHole) {
      if (!holeFilter(holeId) || present.length === 0) continue;
      ...
      if (usesLiveKeepPicker) {
        ...
        const effectiveKeepCount = isIrishFormat(format) ? irishRumbleKeepCountForHole(holeId) : keepCount;
  ...
  const [rosterRows] = await pool.query<any[]>(
    `SELECT tgp.TeamNumber, CONCAT(p.LastName, ', ', p.FirstName) AS name
     FROM TeamGamePlayer tgp
     INNER JOIN Player p ON p.PlayerID = tgp.PlayerID
     WHERE tgp.TeamGameID = ?
     ORDER BY tgp.TeamNumber, p.LastName, p.FirstName`,
  ...
  for (const r of rosterRows) {
    if (!rosterByTeam.has(r.TeamNumber)) rosterByTeam.set(r.TeamNumber, []);
    rosterByTeam.get(r.TeamNumber)!.push(r.name);
  }

  function scoreVsPar(net: number, par: number): string {
    const diff = net - par;
    return diff === 0 ? 'Even' : diff > 0 ? `+${diff}` : `${diff}`;
  }

  const teamNumbers = new Set<number>([...holeScores.keys(), ...rosterByTeam.keys()]);
...
export interface TeamGameCutSummary {
  /** The applicable Net Score to Make Cut value for this team game's own slot, or null if no
   * cut is configured for it (or this is a one-off team game with no Options card at all). */
  cutLine: number | null;
  /** Players who finished but missed the cut, sorted net LOW to HIGH — same order as
   * randomTeamsService.ts's legacy getCutSummary. */
  missedCut: { name: string; total: number }[];
}
 ...
 */
export async function getTeamGameCutSummary(teamGameId: number): Promise<TeamGameCutSummary> {
  const [tgRows] = await pool.query<any[]>('SELECT GameID, Slot, NetCut, NetCut9 FROM TeamGame WHERE TeamGameID = ?', [teamGameId]);
  if (tgRows.length === 0) return { cutLine: null, missedCut: [] };
  const { GameID: gameId, Slot: slot, NetCut: netCut, NetCut9: netCut9 } = tgRows[0];
  if (!slot && netCut === null && netCut9 === null) return { cutLine: null, missedCut: [] };

  const eligibility = slot
    ? await getGameEligibility(gameId, slotPrefix(slot))
    : await getGameEligibility(gameId, 'none', {
...
/**
 * One team's hole-by-hole net scores within a team game's scorecard drill-down — mirrors
 * teamService.ts's getTeamScorecard, scoped to TeamGamePlayer/TeamGameID/TeamNumber. Also marks
 * `paddedHoles` per player: holes where this team was short-handed (Playing Partner Teams, an
 * uneven foursome) and this player's score was the one borrowed a second time to match the
 * game's largest team — see pickPadPlayerIds's docblock. Uses the exact same deterministic
 * pick as getTeamGameResults so the scorecard always agrees with the scoring.
 */
export interface TeamGameScorecardResult {
  rows: TeamScorecardRow[];
  /** This team's actual kept-best net total per hole (same reduction getTeamGameResults uses),
   * keyed by HoleID — shown as a "Total" row below the per-player grid so the hole-by-hole
   * math behind the Front/Back/Total figure is visible, not just the final side total. */
  holeTotals: Record<number, number>;
}

export async function getTeamGameScorecard(
  teamGameId: number,
  teamNumber: number,
  side: 'F' | 'B' | 'T'
): Promise<TeamGameScorecardResult> {
  const holeStart = side === 'B' ? 10 : 1;
  const holeEnd = side === 'F' ? 9 : 18;

  const [tgRows] = await pool.query<any[]>('SELECT GameID, KeepCount, LastHoleAll, Format FROM TeamGame WHERE TeamGameID = ?', [teamGameId]);
  if (tgRows.length === 0) return { rows: [], holeTotals: {} };
  const gameId = tgRows[0].GameID;
  const keepCount = tgRows[0].KeepCount;
  const lastHoleAll = tgRows[0].LastHoleAll === 'T';
  const format = (tgRows[0].Format as string) || 'custom';
  ...
  let holeKeep = new Map<number, number>();
  if (usesLiveKeepPicker) {
    const [keepRows] = await pool.query<any[]>(
      'SELECT HoleID, KeepCount FROM TeamGameHoleKeep WHERE TeamGameID = ? AND TeamNumber = ?',
      [teamGameId, teamNumber]
    );
    holeKeep = new Map(keepRows.map((r) => [r.HoleID, r.KeepCount]));
  }

  const [rows] = await pool.query<any[]>(
    `SELECT sc.PlayerID, CONCAT(p.LastName, ', ', p.FirstName) AS name, sc.HoleID, sc.NetScore
     FROM Score sc
     ...
     ORDER BY sc.PlayerID, sc.HoleID`,
    [teamGameId, teamNumber, gameId, holeStart, holeEnd]
  );

  const byPlayer = new Map<number, TeamScorecardRow>();
  ...
  const ghostRows: TeamScorecardRow[] = Array.from({ length: maxPadCount }, (_, g) => ({
  ...
  const lastHole = lastHoleAll ? 18 : null;
  const holeTotals: Record<number, number> = {};
 ...
 * (app/game.tsx) needs to render itself and validate a choice. `teamSize`/`target` are always
 * derived from this team's actual headcount (whoever's really playing together), never a size
 * fixed in advance -- so a 15-golfer night with three foursomes and one trio just works: the
 * foursomes target 48, the trio targets 36, automatically. */
 ...
 */
export async function getKeepFormatTeamsForPlayers(gameId: number, playerIds: number[]): Promise<KeepFormatTeam[]> {
  if (playerIds.length === 0) return [];

  const [rows] = await pool.query<any[]>(
    `SELECT tg.TeamGameID, tg.Format, tg.Label, tgp.TeamNumber, tgp.PlayerID
     FROM TeamGame tg
     INNER JOIN TeamGamePlayer tgp ON tgp.TeamGameID = tg.TeamGameID
     WHERE tg.GameID = ? AND tg.Format IN ('36/48', '36', '48')`,
    [gameId]
  );

  const byTeam = new Map<string, { teamGameId: number; teamNumber: number; label: string; playerIds: Set<number> }>();
  for (const r of rows) {
  ...
  for (const team of byTeam.values()) {
    ...
    if (isMatch) {
      ...
      result.push({
        ...
        teamSize,
        target: targetForTeamSize(teamSize),
        label: team.label,
      });
    }
  }
...
/** One Irish Rumble team this exact foursome is registered as -- includes the roster's player
 * IDs (unlike KeepFormatTeam) so app/game.tsx can look up each rostered player's own net score
 * for the current hole locally and compute a live "score for this hole" without a round trip. */
 ...
 */
export async function getIrishRumbleTeamsForPlayers(gameId: number, playerIds: number[]): Promise<IrishRumbleTeam[]> {
  if (playerIds.length === 0) return [];

  const [rows] = await pool.query<any[]>(
    `SELECT tg.TeamGameID, tg.Label, tgp.TeamNumber, tgp.PlayerID
     FROM TeamGame tg
     INNER JOIN TeamGamePlayer tgp ON tgp.TeamGameID = tg.TeamGameID
     WHERE tg.GameID = ? AND tg.Format = ?`,
    [gameId, IRISH_RUMBLE_FORMAT]
  );
...
export async function getTeamGameHoleKeepCounts(teamGameId: number, teamNumber: number): Promise<Record<number, number>> {
  const [rows] = await pool.query<any[]>(
    'SELECT HoleID, KeepCount FROM TeamGameHoleKeep WHERE TeamGameID = ? AND TeamNumber = ?',
    [teamGameId, teamNumber]
  );
  const result: Record<number, number> = {};
  for (const r of rows) result[r.HoleID] = r.KeepCount;
  return result;
}
 ...
 */
export async function saveTeamGameHoleKeepCount(
  teamGameId: number,
  teamNumber: number,
  holeId: number,
  keepCount: number,
  holesRemainingIncludingThisOne: number
): Promise<{ ok: boolean; error?: string }> {
  const [tgRows] = await pool.query<any[]>('SELECT Format FROM TeamGame WHERE TeamGameID = ?', [teamGameId]);
  if (tgRows.length === 0) return { ok: false, error: 'Team game not found.' };
  const format = tgRows[0].Format as string;
  if (!isKeepFormat(format)) return { ok: false, error: 'This team game is not a 36/48 format.' };

  // Team size is always this specific team's actual headcount, not a value fixed in advance.
  const [rosterRows] = await pool.query<any[]>(
    'SELECT COUNT(DISTINCT PlayerID) AS cnt FROM TeamGamePlayer WHERE TeamGameID = ? AND TeamNumber = ?',
    [teamGameId, teamNumber]
  );
  const teamSize = Number(rosterRows[0]?.cnt) || 0;
  const target = targetForTeamSize(teamSize);

  const [otherRows] = await pool.query<any[]>(
    'SELECT KeepCount FROM TeamGameHoleKeep WHERE TeamGameID = ? AND TeamNumber = ? AND HoleID != ?',
    [teamGameId, teamNumber, holeId]
  );
  const keptOnOtherHoles = otherRows.reduce((sum, r) => sum + Number(r.KeepCount), 0);

  const validChoices = computeValidKeepChoices(teamSize, target, keptOnOtherHoles, holesRemainingIncludingThisOne);
  if (!validChoices.includes(keepCount)) {
    return { ok: false, error: `${keepCount} isn't a valid choice for this hole.` };
  }

  await pool.query(
    `INSERT INTO TeamGameHoleKeep (TeamGameID, TeamNumber, HoleID, KeepCount, LastUpdateUser)
     VALUES (?, ?, ?, ?, 'App')
     ON DUPLICATE KEY UPDATE KeepCount = VALUES(KeepCount), LastUpdateDt = CURRENT_TIMESTAMP, LastUpdateUser = VALUES(LastUpdateUser)`,
    [teamGameId, teamNumber, holeId, keepCount]
  );
  return { ok: true };
}

/** A score-vs-par figure paired with how many net scores went into it — a bare "+3" or "-2"
 * doesn't mean anything on its own for this format (a team that's kept 30 scores and one that's
 * kept 20 aren't comparable without knowing that), so the kept count always travels with it. */
export interface TeamLiveSideScore {
  /** 'Even' | '+N' | '-N' | '-' (no holes finished on this side yet). */
  score: string;
  kept: number;
}

/** One 36/48 team's live standing — deliberately simple for now (no "leaderboard lag" to hide
 * ahead-of-pace teams from behind-pace ones): just enough for every team to see how they're
 * doing without over-engineering a fairness mechanism that may not end up mattering in practice. */
export interface TeamLiveLeaderboardRow {
  teamNumber: number;
  players: string[];
  /** Holes with a recorded live keep-count choice — NOT just holes with a saved gross score,
   * since the picker hard-blocks advancing, so this is the true "holes this team has actually
   * finished," including their keep decision. */
  thru: number;
 ...
 */
export async function getTeamGameLiveLeaderboard(teamGameId: number): Promise<TeamLiveLeaderboardRow[]> {
  const [tgRows] = await pool.query<any[]>('SELECT GameID, Format FROM TeamGame WHERE TeamGameID = ?', [teamGameId]);
  if (tgRows.length === 0) return [];
  const gameId = tgRows[0].GameID;
  const format = tgRows[0].Format as string;
  if (!isKeepFormat(format)) return [];

  const [gameRows] = await pool.query<any[]>('SELECT CourseID FROM Game WHERE GameID = ?', [gameId]);
  const courseId = gameRows[0]?.CourseID;
  const [holeRows] = await pool.query<any[]>('SELECT HoleNum, Par FROM CourseDetails WHERE CourseID = ?', [courseId]);
  const parByHole = new Map<number, number>(holeRows.map((h) => [h.HoleNum, h.Par]));

  const [keepRows] = await pool.query<any[]>(
    'SELECT TeamNumber, HoleID, KeepCount FROM TeamGameHoleKeep WHERE TeamGameID = ?',
    [teamGameId]
  );
  const keepByTeam = new Map<number, Map<number, number>>();
  for (const r of keepRows) {
    if (!keepByTeam.has(r.TeamNumber)) keepByTeam.set(r.TeamNumber, new Map());
  ...
  const [rosterRows] = await pool.query<any[]>(
    `SELECT tgp.TeamNumber, CONCAT(p.LastName, ', ', p.FirstName) AS name
     ...
     WHERE tgp.TeamGameID = ? ORDER BY tgp.TeamNumber, p.LastName, p.FirstName`,
    [teamGameId]
  );
  const rosterByTeam = new Map<number, string[]>();
  for (const r of rosterRows) {
  ...
  const [scoreRows] = await pool.query<any[]>(
    `SELECT tgp.TeamNumber, s.HoleID, s.NetScore
     FROM TeamGamePlayer tgp
     INNER JOIN Score s ON s.PlayerID = tgp.PlayerID AND s.GameID = ?
     WHERE tgp.TeamGameID = ?`,
    [gameId, teamGameId]
  );
  const netByTeamHole = new Map<number, Map<number, number[]>>();
  for (const r of scoreRows) {
    if (!netByTeamHole.has(r.TeamNumber)) netByTeamHole.set(r.TeamNumber, new Map());
  ...
  function computeSide(
    ...
    netMap: Map<number, number[]>,
    holeFilter: (holeId: number) => boolean
  ): TeamLiveSideScore {
    let netTotal = 0;
    let parTotal = 0;
    let kept = 0;
    for (const [holeId, keepCount] of keepMap) {
      if (!holeFilter(holeId)) continue;
  ...
  const teamNumbers = new Set<number>([...rosterByTeam.keys(), ...keepByTeam.keys()]);

  const result: TeamLiveLeaderboardRow[] = Array.from(teamNumbers)
    .sort((a, b) => a - b)
    .map((teamNumber) => {
      const keepMap: Map<number, number> = keepByTeam.get(teamNumber) || new Map();
      const netMap: Map<number, number[]> = netByTeamHole.get(teamNumber) || new Map();
      ...
      return { teamNumber, players, thru: keepMap.size, front, back, total, target: targetForTeamSize(players.length) };
    });
 ...
 * player choice), just how many holes they've finished and their cumulative net-vs-par so far.
 * `score` is the whole round's total (same value front+back would sum to); `front`/`back` are the
 * two 9s split out for the summary card on game.tsx's Front 9 / Back 9 / Overall screens. */
 ...
 */
export async function getFixedKeepLiveLeaderboard(teamGameId: number): Promise<FixedKeepLiveRow[]> {
  const [tgRows] = await pool.query<any[]>('SELECT GameID, Format, KeepCount, LastHoleAll FROM TeamGame WHERE TeamGameID = ?', [teamGameId]);
  if (tgRows.length === 0) return [];
  const gameId = tgRows[0].GameID;
  const format = tgRows[0].Format as string;
  if (format !== 'custom' && !isIrishFormat(format)) return [];
  const keepCount = tgRows[0].KeepCount;
  const lastHoleAll = tgRows[0].LastHoleAll === 'T';
  ...
  const teamNumbers = new Set<number>([...rosterByTeam.keys(), ...holeScores.keys()]);
  ...
  const result: FixedKeepLiveRow[] = Array.from(teamNumbers)
    .sort((a, b) => a - b)
    .map((teamNumber) => {
      const perHole: Map<number, { playerId: number; net: number }[]> = holeScores.get(teamNumber) || new Map();
...
export async function getBestPossibleWeeks(eventId: number): Promise<{ gameId: number; gameDate: string }[]> {
  const [rows] = await pool.query<any[]>(
    `SELECT DISTINCT g.GameID, g.GameDate
     FROM Game g
...
/** Actual result vs. the theoretical best a 36/48 team's round could have produced, in
 * hindsight -- a post-round-only "What If" for a fixed team (no re-pairing, unlike the
 * individual What If screen, since Playing Partner Teams already fixed the roster). One row
 * per real team in the team game -- shown together on one screen, not picked one at a time. */
 ...
 * scores count.
 */
export async function getTeamBestPossible(teamGameId: number): Promise<TeamBestPossible[]> {
  const [tgRows] = await pool.query<any[]>('SELECT GameID, Format FROM TeamGame WHERE TeamGameID = ?', [teamGameId]);
  if (tgRows.length === 0) return [];
  const gameId = tgRows[0].GameID;
  const format = tgRows[0].Format as string;
  ...
  const [rosterRows] = await pool.query<any[]>(
    `SELECT tgp.TeamNumber, tgp.PlayerID, CONCAT(p.LastName, ', ', p.FirstName) AS name
     FROM TeamGamePlayer tgp
     INNER JOIN Player p ON p.PlayerID = tgp.PlayerID
     WHERE tgp.TeamGameID = ?
     ORDER BY tgp.TeamNumber, p.LastName, p.FirstName`,
  ...
  const playersByTeam = new Map<number, { playerId: number; name: string }[]>();
  for (const r of rosterRows) {
    if (!playersByTeam.has(r.TeamNumber)) playersByTeam.set(r.TeamNumber, []);
  ...
  const netsByPlayer = new Map<number, { holeId: number; net: number }[]>();
  for (const r of scoreRows) {
    if (!netsByPlayer.has(r.PlayerID)) netsByPlayer.set(r.PlayerID, []);
  ...
  const keepByTeam = new Map<number, { holeId: number; keepCount: number }[]>();
  for (const r of keepRows) {
    if (!keepByTeam.has(r.TeamNumber)) keepByTeam.set(r.TeamNumber, []);
    keepByTeam.get(r.TeamNumber)!.push({ holeId: r.HoleID, keepCount: r.KeepCount });
  }

  function scoreVsPar(totalNet: number, totalPar: number): string {
    const diff = totalNet - totalPar;
    return diff === 0 ? 'Even' : diff > 0 ? `+${diff}` : `${diff}`;
  }

  const results: (TeamBestPossible & { bestDiffSum: number })[] = [];
  for (const [teamNumber, teamPlayers] of playersByTeam) {
    const teamSize = teamPlayers.length;
    const target = targetForTeamSize(teamSize);

    const allDiffs: number[] = [];
    const netsByHole = new Map<number, number[]>();
    for (const { playerId } of teamPlayers) {
      for (const { holeId, net } of netsByPlayer.get(playerId) || []) {
        allDiffs.push(net - (parByHole.get(holeId) ?? 0));
        if (!netsByHole.has(holeId)) netsByHole.set(holeId, []);
