...
import { resolveTeamGameId, getOrCreateDefaultTeamGameId } from './teamService';

export interface RandomTeamsGameInfo {
  gameId: number;
  gameDate: string;
  alreadySet: boolean;
  /** Names of active players who haven't finished all their holes yet. */
  stillPlaying: string[];
}
 ...
 */
export async function getLatestGameForRandomTeams(eventId: number): Promise<RandomTeamsGameInfo | null> {
  const [rows] = await pool.query<any[]>(
    `SELECT DISTINCT g.GameID, g.GameDate, g.RandomSet
     FROM Game g
 ...
 */
export async function getRandomTeamsListing(gameId: number): Promise<RandomTeamResult[]> {
  const teamGameId = await resolveTeamGameId(gameId);
  if (teamGameId === null) return [];

  const [teamRows] = await pool.query<any[]>(
    `SELECT t.TeamNumber AS TeamID, t.PlayerID, CONCAT(p.LastName, ', ', p.FirstName) AS name, g.GameDate
     ...
     WHERE t.TeamGameID = ?
     ORDER BY t.TeamNumber, p.LastName, p.FirstName`,
    [teamGameId]
  );
  ...
  for (const r of teamRows) {
    if (!byTeam.has(r.TeamID)) byTeam.set(r.TeamID, { teamId: r.TeamID, players: [] });
...
type HoleShape = 'full18' | 'front9' | 'back9' | 'incomplete';

interface PlayerShape {
  playerId: number;
  name: string;
  shape: HoleShape;
  /** Net total for whichever side matches this player's shape (0 for 'incomplete'). */
  total: number;
}
...
function strictShape(c: PlayerCounts): HoleShape {
  if (c.front === 9 && c.back === 9) return 'full18';
  if (c.front === 9 && c.back === 0) return 'front9';
  if (c.back === 9 && c.front === 0) return 'back9';
  return 'incomplete';
}
 ...
 */
async function getPlayerCounts(gameId: number): Promise<PlayerCounts[]> {
  const [rows] = await pool.query<any[]>(
    `SELECT s.PlayerID, CONCAT(p.LastName, ', ', p.FirstName) AS name,
     ...
     GROUP BY s.PlayerID, p.LastName, p.FirstName`,
    [gameId, gameId]
  );
...
export interface GameEligibility {
  /** Players who haven't completed a clean 18/front-9/back-9 shape yet. */
  stillPlaying: PlayerShape[];
  /** The hole shape most players finished with — used to pick which cut applies. */
  majorityShape: HoleShape | null;
  /** Finished, matched the majority shape, and under (or no) cut — the players who get randomized. */
  eligiblePlayerIds: number[];
  /** Finished but didn't match the majority shape (e.g. entered the wrong side and never fixed it). */
  excludedWrongSide: PlayerShape[];
  /** Finished, matched the majority shape, but over the cut. */
  excludedOverCut: PlayerShape[];
  /** The applicable Net Score to Make Cut value, or null if no cut applies. */
  cut: number | null;
}
...
export async function getGameEligibility(
  gameId: number,
  cutOptionPrefix: string = 'teams',
  directCut?: { full18: number | null; nineHole: number | null }
  ...
  const shapePriority: HoleShape[] = ['full18', 'front9', 'back9'];
  let majorityShape: HoleShape | null = null;
  let bestCount = 0;
  ...
  const stillPlaying: PlayerShape[] = [];
  const matchingMajority: PlayerShape[] = [];
  const excludedWrongSide: PlayerShape[] = [];
  ...
  if (majorityShape === 'full18') {
    ...
    for (const c of counts) stillPlaying.push({ playerId: c.playerId, name: c.name, shape: 'incomplete', total: 0 });
  }
  ...
  if (directCut) {
    ...
    const cutKey = (majorityShape === 'full18' ? `${cutOptionPrefix}_netcut` : `${cutOptionPrefix}_netcut9`) as keyof NonNullable<typeof options>;
    const cutValue = options?.[cutKey];
    cut = cutValue && cutValue !== '' ? Number(cutValue) : null;
  }

  const excludedOverCut: PlayerShape[] = [];
 ...
 */
export async function getCutSummary(gameId: number): Promise<CutSummary> {
...
export interface CreateRandomTeamsResult {
  ...
  /** Present (and teams left empty) when not everyone has finished yet. */
  stillPlaying?: string[];
  /** True (and teams left empty) when everyone eligible has finished, but fewer than 4 players
   * are eligible to draw — 2-person teams need at least 2 teams (4 players) to actually be a
   * "teams game"; 2 or 3 eligible players would draw a single lone team with no one to compete
   * against. Does NOT set RandomSet, so the draw can simply be retried later once more players
   * are eligible, instead of permanently locking in a broken/meaningless team. */
 ...
 */
export async function createRandomTeams(gameId: number): Promise<CreateRandomTeamsResult> {
  const [gameRows] = await pool.query<any[]>('SELECT RandomSet FROM Game WHERE GameID = ?', [gameId]);
  ...
  await pool.query(`UPDATE Game SET RandomSet = 'T' WHERE GameID = ?`, [gameId]);
