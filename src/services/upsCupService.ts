...
export interface IneligiblePlayer {
  ...
  remaining: number;
  reason: string;
  /** Whether this player has paid their UPS Cup entry for this year -- the caller (Standings
   * screen) hides unpaid players here by default, same as it already does for the main
   * standings list, revealed by its "Show unpaid players" toggle. */
  paid: boolean;
}
  ...
  if (numScores > 0 && numPlayers > 0) {
    ...
    if (paidAverages.length > 0) {
      ...
      for (const [playerId, { name, points }] of pointsByPlayer) {
        if (alreadyFlagged.has(playerId)) continue;
