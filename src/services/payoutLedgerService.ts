...
import { computePlaceAmounts, computeTiePayouts } from '../utils/payout';
import { truncateMoneyValue } from '../utils/money';
import { getWeekResults } from './weekResultsService';
import { getEventOptions, getEffectiveGamePayoutOptions, PayoutOverrideKey } from './optionsService';
import { getGameEligibility } from './randomTeamsService';
import { getTeamGameResults } from './teamGameService';

export type PayoutGameType = 'net' | 'teams' | 'teams2' | 'teams3' | 'teams4' | 'skins' | 'grossskins' | 'holeinone';

const TEAM_SLOTS: { prefix: Exclude<PayoutGameType, 'net' | 'skins' | 'grossskins' | 'holeinone'>; slot: number }[] = [
  { prefix: 'teams', slot: 1 },
...
async function getGameEventId(gameId: number): Promise<number | null> {
  const [rows] = await pool.query<any[]>('SELECT GroupID FROM Game WHERE GameID = ?', [gameId]);
...
async function syncOnePayoutType(gameId: number, gameType: PayoutGameType, amounts: Map<number, number>): Promise<void> {
  await pool.query('DELETE FROM PlayerPayouts WHERE GameID = ? AND GameType = ?', [gameId, gameType]);
  const entries = Array.from(amounts.entries())
    .map(([playerId, amount]) => [playerId, truncateMoneyValue(amount)] as const)
    .filter(([, amt]) => amt > 0.001);
  if (entries.length === 0) return;
  const values = entries.map(([playerId, amount]) => [gameId, playerId, gameType, amount]);
  await pool.query('INSERT INTO PlayerPayouts (GameID, PlayerID, GameType, Amount) VALUES ?', [values]);
}

async function syncNetPayout(
  gameId: number,
  eventOptions: Record<string, any>,
  payoutValues: Record<PayoutOverrideKey, string>
): Promise<void> {
  const netResults = await getWeekResults(gameId, 'N');
  ...
  const pctArr = Array.from({ length: places }, (_, i) => payoutValues[`game_netpct${i + 1}` as PayoutOverrideKey] ?? '');
  const placeAmounts = computePlaceAmounts(pot, places, pctArr);
  const tiePayouts = computeTiePayouts(netResults.map((r) => ({ key: r.playerId, value: r.net })), placeAmounts);
 ...
 */
async function syncOneTeamSlotPayout(
  gameId: number,
  prefix: Exclude<PayoutGameType, 'net' | 'skins' | 'grossskins' | 'holeinone'>,
  slot: number,
  eventOptions: Record<string, any>,
  payoutValues: Record<PayoutOverrideKey, string>
): Promise<void> {
  const [tgRows] = await pool.query<any[]>(
    slot === 1
      ...
      : 'SELECT TeamGameID FROM TeamGame WHERE GameID = ? AND Slot = ? ORDER BY TeamGameID LIMIT 1',
    slot === 1 ? [gameId] : [gameId, slot]
  );
  ...
  const totalsRows = (await getTeamGameResults(teamGameId)).map((r) => ({ TeamNumber: r.teamId, total: r.total }));
  ...
  // cut or no cut.
  const eligibility = await getGameEligibility(gameId, prefix);
  ...
  const places = Math.max(0, Math.min(10, Number(payoutValues[`${prefix}_places` as PayoutOverrideKey]) || 0));
  const pctArr = Array.from({ length: places }, (_, i) => payoutValues[`${prefix}_pct${i + 1}` as PayoutOverrideKey] ?? '');
  const placeAmounts = computePlaceAmounts(pot, places, pctArr);
  const teamPayouts = computeTiePayouts(
    totalsRows.map((t) => ({ key: t.TeamNumber, value: Number(t.total) })),
    placeAmounts
  );

  const perPlayerAmounts = new Map<number, number>();
 ...
 */
async function syncOneOffTeamGamePayout(gameId: number, teamGameId: number): Promise<void> {
  const gameType = `oneoff${teamGameId}`;
  const [tgRows] = await pool.query<any[]>(
    'SELECT PayIn, Places, Pct1, Pct2, Pct3, Pct4 FROM TeamGame WHERE TeamGameID = ?',
    [teamGameId]
  );
  ...
  const eligibility = await getGameEligibility(gameId, 'none', { full18: netCut, nineHole: netCut9 });
  const potPlayerCount = eligibility.eligiblePlayerIds.length + eligibility.excludedOverCut.length;
  const pot = payIn * potPlayerCount;
...
async function syncSkinsPayout(gameId: number, eventOptions: Record<string, any>): Promise<void> {
  const [countRows] = await pool.query<any[]>(
    `SELECT COUNT(DISTINCT s.PlayerID) AS numPlayers,
  ...
  const [skinsRows] = await pool.query<any[]>(
    `SELECT sk.PlayerID, COUNT(*) AS skinCount
     FROM Skins sk
     WHERE sk.GameID = ? AND sk.Validated = 'T'
       AND sk.PlayerID NOT IN (SELECT o.PlayerID FROM OptOut o WHERE o.GameID = ?)
     GROUP BY sk.PlayerID`,
    [gameId, gameId]
  );
  const amounts = new Map<number, number>(skinsRows.map((r) => [r.PlayerID, r.skinCount * perSkin]));
...
async function syncGrossSkinsPayout(gameId: number, groupId: number, gameDate: Date, eventOptions: Record<string, any>): Promise<void> {
  ...
  const [skinsRows] = await pool.query<any[]>(
    `SELECT gs.PlayerID, COUNT(*) AS skinCount FROM GrossSkinsResult gs WHERE gs.GameID = ? AND gs.Validated = 'T' GROUP BY gs.PlayerID`,
    [gameId]
  );
  const amounts = new Map<number, number>(skinsRows.map((r) => [r.PlayerID, r.skinCount * perSkin]));
...
/** A player who holed out in one stroke this game -- the rarest of rare events, and by house
 * rule wins the ENTIRE day's pot (Net + every Teams slot + Skins combined), overriding every
 * normal payout split (confirmed with Matt 2026-07-26, real case: a hole-in-one on hole 7).
 * Multiple in the same game (vanishingly unlikely, but not impossible) split the pot evenly. */
...
export async function getHoleInOneWinners(gameId: number): Promise<HoleInOneWinner[]> {
  const [rows] = await pool.query<any[]>(
    `SELECT s.PlayerID AS playerId, CONCAT(p.LastName, ', ', p.FirstName) AS name, s.HoleID AS holeId
     FROM Score s
...
async function computeTotalDayPot(gameId: number, eventOptions: Record<string, any>): Promise<number> {
  let total = 0;
  ...
  for (const { prefix, slot } of TEAM_SLOTS) {
    ...
    if (tgRows.length === 0) continue;
    const [rosterRows] = await pool.query<any[]>(
      'SELECT DISTINCT PlayerID FROM TeamGamePlayer WHERE TeamGameID = ?',
      [tgRows[0].TeamGameID]
    );
  ...
  if (skinsPayIn > 0) {
    const [countRows] = await pool.query<any[]>(
      `SELECT COUNT(DISTINCT s.PlayerID) AS numPlayers FROM Score s
...
/** Combined hole-in-one info for the Week Results celebration screen -- null when this game had
 * no hole-in-one, so the caller can skip the celebration entirely without a second round trip. */
export interface HoleInOneCelebration {
  winners: HoleInOneWinner[];
  totalPot: number;
}

export async function getHoleInOneCelebration(gameId: number): Promise<HoleInOneCelebration | null> {
  const winners = await getHoleInOneWinners(gameId);
  if (winners.length === 0) return null;
  const eventId = await getGameEventId(gameId);
  if (eventId === null) return null;
  const eventOptions = await getEventOptions(eventId);
  const totalPot = await computeTotalDayPot(gameId, eventOptions);
  return { winners, totalPot };
}

export interface PurseGameTypeAmount {
  gameType: PayoutGameType;
  amount: number;
}
 ...
 */
export async function getWeekPurse(gameId: number): Promise<PurseRow[]> {
  const [rows] = await pool.query<any[]>(
    `SELECT pp.PlayerID, CONCAT(p.LastName, ', ', p.FirstName) AS name, pp.GameType, pp.Amount
  ...
  const byPlayer = new Map<number, PurseRow>();
  for (const r of rows) {
 ...
 */
export async function syncGamePayoutLedger(gameId: number): Promise<void> {
  const eventId = await getGameEventId(gameId);
  if (eventId === null) return;

  const eventOptions = await getEventOptions(eventId);
  const { values: payoutValues } = await getEffectiveGamePayoutOptions(gameId, eventId);
  ...
  const [oneOffRows] = await pool.query<any[]>(
    `SELECT TeamGameID FROM TeamGame WHERE GameID = ? AND Slot IS NULL AND (Skipped IS NULL OR Skipped != 'T')`,
    [gameId]
  );
 ...
 */
export async function getPayoutReviewForEvent(eventId: number): Promise<PayoutReviewWeek[]> {
  const [calRows] = await pool.query<any[]>(
    `SELECT DISTINCT DATE_FORMAT(EventDate, '%Y-%m-%d') AS date
     FROM EventCalendar WHERE EventID = ? AND DayType != 'note' AND EventDate <= CURDATE()`,
    [eventId]
  );
  let qualifyingDates: Set<string> | null = calRows.length > 0 ? new Set(calRows.map((r) => r.date)) : null;

  const [gameRows] = await pool.query<any[]>(
    `SELECT GameID, DATE_FORMAT(GameDate, '%Y-%m-%d') AS date FROM Game
     WHERE GroupID = ? AND GameDate <= CURDATE() ORDER BY GameDate DESC`,
    [eventId]
  );
  ...
  if (qualifyingGames.length === 0) return [];

  const eventOptions = await getEventOptions(eventId);
  const netPayIn = Number(eventOptions.game_netpayin) || 0;

  const weeks: PayoutReviewWeek[] = [];
  for (const g of qualifyingGames) {
    const { values: payoutValues, overriddenKeys } = await getEffectiveGamePayoutOptions(g.GameID, eventId);

    let netTotal = 0;
    if (netPayIn > 0) {
      const netResults = await getWeekResults(g.GameID, 'N');
      if (netResults.length > 0) {
        const places = Math.max(0, Math.min(10, Number(payoutValues.game_netplaces) || 0));
        const pctArr = Array.from({ length: places }, (_, i) => payoutValues[`game_netpct${i + 1}` as PayoutOverrideKey] ?? '');
        netTotal = computePlaceAmounts(netPayIn * netResults.length, places, pctArr).reduce((a, b) => a + b, 0);
      }
    }
    ...
    for (const { prefix, slot } of TEAM_SLOTS) {
      const payIn = Number((eventOptions as any)[`${prefix}_payin`]) || 0;
      if (payIn <= 0) continue;
      const [tgRows] = await pool.query<any[]>(
        slot === 1
          ? 'SELECT TeamGameID FROM TeamGame WHERE GameID = ? AND (Slot = 1 OR Slot IS NULL) ORDER BY TeamGameID LIMIT 1'
          : 'SELECT TeamGameID FROM TeamGame WHERE GameID = ? AND Slot = ? ORDER BY TeamGameID LIMIT 1',
        slot === 1 ? [g.GameID] : [g.GameID, slot]
      );
      if (tgRows.length === 0) continue;
      const [rosterRows] = await pool.query<any[]>(
        'SELECT DISTINCT PlayerID FROM TeamGamePlayer WHERE TeamGameID = ?',
      ...
      const places = Math.max(0, Math.min(10, Number(payoutValues[`${prefix}_places` as PayoutOverrideKey]) || 0));
      const pctArr = Array.from({ length: places }, (_, i) => payoutValues[`${prefix}_pct${i + 1}` as PayoutOverrideKey] ?? '');
      teamsTotal += computePlaceAmounts(payIn * rosterRows.length, places, pctArr).reduce((a, b) => a + b, 0);
      if (overriddenKeys.some((k) => k.startsWith(`${prefix}_`))) teamsOverridden = true;
    }
 ...
 */
export async function getSeasonPayoutSummary(eventId: number): Promise<SeasonPayoutRow[]> {
  const [wonRows] = await pool.query<any[]>(
    `SELECT pp.PlayerID, SUM(pp.Amount) AS won
     FROM PlayerPayouts pp
  ...
  const [netPaidRows] = await pool.query<any[]>(
    `SELECT s.PlayerID, COUNT(DISTINCT s.GameID) AS gamesPlayed
     FROM Score s
  ...
  const [skinsPaidRows] = await pool.query<any[]>(
    `SELECT s.PlayerID, COUNT(DISTINCT s.GameID) AS gamesPlayed
     FROM Score s
     INNER JOIN Game g ON g.GameID = s.GameID
     WHERE g.GroupID = ?
  ...
  const [teamAppearanceRows] = await pool.query<any[]>(
    `SELECT DISTINCT tgp.PlayerID, tg.TeamGameID, tg.Slot, tg.PayIn
  ...
  const [grossSkinsPaidRows] = await pool.query<any[]>(
    `SELECT PlayerID, COUNT(*) AS gamesPlayed FROM GSkinsPaid WHERE GroupID = ? GROUP BY PlayerID`,
    [eventId]
  );
  ...
  const addPaid = (rows: any[], rate: number) => {
    for (const r of rows) {
  ...
  for (const r of teamAppearanceRows) {
    ...
    const rate = r.Slot === null || r.Slot === undefined
      ? Number(r.PayIn) || 0
      : Number((eventOptions as any)[`${r.Slot === 1 ? 'teams' : `teams${r.Slot}`}_payin`]) || 0;
    if (rate > 0) {
      paidById.set(r.PlayerID, (paidById.get(r.PlayerID) ?? 0) + rate);
    }
  }

  const wonById = new Map<number, number>(wonRows.map((r) => [r.PlayerID, Number(r.won)]));

  const allPlayerIds = new Set<number>([...paidById.keys(), ...wonById.keys()]);
  if (allPlayerIds.size === 0) return [];

  const [nameRows] = await pool.query<any[]>(
    `SELECT PlayerID, CONCAT(LastName, ', ', FirstName) AS name FROM Player WHERE PlayerID IN (${Array.from(allPlayerIds).map(() => '?').join(',')})`,
    Array.from(allPlayerIds)
  );
  const nameById = new Map<number, string>(nameRows.map((r) => [r.PlayerID, r.name]));
