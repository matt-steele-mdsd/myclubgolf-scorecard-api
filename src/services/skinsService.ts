...
import { getStrokes, hdcpForPlayer } from '../utils/handicap';
import { getGenderedHoleHdcps, getPlayerGenders } from './gameService';
import { truncateMoneyValue } from '../utils/money';

type SkinsStrokeMode = 'none' | 'half' | 'full';
...
function resolveSkinsModes(options: Awaited<ReturnType<typeof getEventOptions>>): {
  par3Mode: SkinsStrokeMode;
  par45Mode: SkinsStrokeMode;
  maxOneStrokeSkins: boolean;
} {
  const par3Mode: SkinsStrokeMode = options?.skins_nonepar3 ? 'none' : options?.skins_halfpar3 ? 'half' : 'full';
  const par45Mode: SkinsStrokeMode = options?.skins_maxone ? 'none' : options?.skins_halfall ? 'half' : 'full';
...
function computeSkinsScoreValue(
  gross: number,
  par: number,
  holeHdcp: number,
  playerHdcp: number,
  modes: { par3Mode: SkinsStrokeMode; par45Mode: SkinsStrokeMode; maxOneStrokeSkins: boolean }
): number {
  const strokes = getStrokes(playerHdcp, holeHdcp);
  const skinsStrokes = modes.maxOneStrokeSkins ? Math.min(strokes, 1) : strokes;
  if (par === 3) {
    if (modes.par3Mode === 'none') return gross;
    if (modes.par3Mode === 'full') return gross - skinsStrokes;
    return gross - skinsStrokes * 0.5;
  }
  if (modes.par45Mode === 'none') return gross;
  if (modes.par45Mode === 'half') return gross - skinsStrokes * 0.5;
...
async function resolvePlayerHdcps(gameId: number, gameDate: Date, playerIds: number[]): Promise<Map<number, number>> {
 ...
 */
export async function getScoredHoles(gameId: number): Promise<number[]> {
  const [rows] = await pool.query<any[]>(
    'SELECT DISTINCT HoleID FROM Score WHERE GameID = ? ORDER BY HoleID',
    [gameId]
  );
...
/**
 * Whether the outright low-score winner "backed up" the skin on the next hole they
 * played, per the event's Validate Skins By option: 'none' skips the check entirely
 * (always validated), 'par' requires a net par or better, 'lowestNet' requires
 * matching the lowest net score anyone shot on that hole.
 */
...
function validationModeFromOptions(options: Awaited<ReturnType<typeof getEventOptions>>): SkinsValidationMode {
  if (options.skins_validation_score) return 'lowestNet';
  if (options.skins_validation_par) return 'par';
  return 'none';
}
 ...
 */
async function getValidationMode(gameId: number): Promise<SkinsValidationMode> {
  const [rows] = await pool.query<any[]>('SELECT GroupID FROM Game WHERE GameID = ?', [gameId]);
  if (rows.length === 0) return 'par';
  const options = await getEventOptions(rows[0].GroupID);
 ...
 */
export async function findFollowUpHole(gameId: number, holeId: number): Promise<number> {
  if (holeId === 18) {
    const [rows] = await pool.query<any[]>('SELECT COUNT(Score) AS cnt FROM Score WHERE GameID = ? AND HoleID = 1', [gameId]);
    return rows[0].cnt > 0 ? 1 : 10;
  }
  if (holeId === 9) {
    const [rows] = await pool.query<any[]>('SELECT COUNT(Score) AS cnt FROM Score WHERE GameID = ? AND HoleID = 10', [gameId]);
    return rows[0].cnt > 0 ? 10 : 1;
  }
  return holeId + 1;
}

/** Game/Options context `recalculateAllSkins` fetches once and hands to every hole's
 * `getSkinsForHole` call, instead of each hole re-fetching the same identical rows. */
export interface SkinsHoleContext {
  groupId: number;
  courseId: number;
  gameDate: Date;
  options: Awaited<ReturnType<typeof getEventOptions>>;
}
 ...
 */
export async function getSkinsForHole(gameId: number, holeId: number, context?: SkinsHoleContext): Promise<SkinsHoleResult> {
  await pool.query('DELETE FROM Skins WHERE GameID = ? AND HoleID = ?', [gameId, holeId]);

  let groupId: number;
  let courseId: number;
  let gameDate: Date;
  let options: Awaited<ReturnType<typeof getEventOptions>>;

  if (context) {
    ({ groupId, courseId, gameDate, options } = context);
  } else {
    const [gameRows] = await pool.query<any[]>('SELECT GroupID, CourseID, GameDate FROM Game WHERE GameID = ?', [gameId]);
    if (gameRows.length === 0) return { rows: [], validation: null };
    ({ GroupID: groupId, CourseID: courseId, GameDate: gameDate } = gameRows[0]);
    options = await getEventOptions(groupId);
  }

  const [holeRows] = await pool.query<any[]>(
    'SELECT Par, Hdcp FROM CourseDetails WHERE CourseID = ? AND HoleNum = ?',
    [courseId, holeId]
  );
  if (holeRows.length === 0) return { rows: [], validation: null };
  const { Par: par, Hdcp: holeHdcp } = holeRows[0];

  const modes = resolveSkinsModes(options);

  const [scoreRows] = await pool.query<any[]>(
    `SELECT p.PlayerID, CONCAT(p.LastName, ', ', p.FirstName) AS name, s.Score, s.NetScore
     FROM Score s
  ...
  const [playerHdcps, playerGenders, genderedHdcps] = await Promise.all([
    ...
    getPlayerGenders(playerIds),
    getGenderedHoleHdcps(courseId),
  ]);
  const computed: { PlayerID: number; name: string; Score: number; NetScore: number; skinsScore: number }[] = [];
  for (const r of scoreRows) {
    const playerHdcp = playerHdcps.get(r.PlayerID) ?? 0;
    const gender = playerGenders.get(r.PlayerID) ?? 'M';
    const playerHoleHdcp = hdcpForPlayer(
      { hdcp: holeHdcp, hdcpMale: genderedHdcps.male.get(holeId) ?? null, hdcpFemale: genderedHdcps.female.get(holeId) ?? null },
      gender,
      options.women_hdcp_holes
    );
  ...
  if (computed.length > 0) {
    ...
    await pool.query(
      `UPDATE Score SET SkinsScore = CASE PlayerID ${caseClauses} END
       WHERE GameID = ? AND HoleID = ? AND PlayerID IN (?)`,
      [...caseParams, gameId, holeId, playerIds]
    );
  }
  ...
  if (rows.length === 1) {
    ...
    if (mode === 'none') {
      ...
      await pool.query(
        `INSERT INTO Skins (GroupID, GameID, HoleID, PlayerID, Score, NetScore, Validated, LastUpdateUser)
         VALUES (?, ?, ?, ?, ?, ?, 'T', 'App')
        ...
        [groupId, gameId, holeId, winner.PlayerID, winner.Score, winner.NetScore]
      );
      ...
      const [followUpRows] = await pool.query<any[]>(
        `SELECT s.Score, s.NetScore, cd.Par
         FROM Score s
      ...
      if (followUpRows.length > 0) {
        ...
        let lowestNet: number | undefined;
        let lowestNetHolder: string | undefined;
        ...
        if (mode === 'par') {
          ...
          const [lowRows] = await pool.query<any[]>(
            `SELECT CONCAT(p.LastName, ', ', p.FirstName) AS name, s.NetScore
             FROM Score s
             INNER JOIN Player p ON p.PlayerID = s.PlayerID
             WHERE s.GameID = ? AND s.HoleID = ?
               AND s.PlayerID NOT IN (SELECT o.PlayerID FROM OptOut o WHERE o.GameID = ?)
        ...
        await pool.query(
          `INSERT INTO Skins (GroupID, GameID, HoleID, PlayerID, Score, NetScore, Validated, LastUpdateUser)
           ...
           ON DUPLICATE KEY UPDATE
             ...
             NetScore = VALUES(NetScore),
             Validated = VALUES(Validated)`,
          [groupId, gameId, holeId, winner.PlayerID, winner.Score, winner.NetScore, validated ? 'T' : 'F']
        );
 ...
 */
export async function invalidateSkinsCache(gameId: number): Promise<void> {
  await pool.query('DELETE FROM SkinsCalcStatus WHERE GameID = ?', [gameId]);
}
 ...
 */
export async function recalculateAllSkins(gameId: number): Promise<void> {
  const [[status]] = await pool.query<any[]>('SELECT GameID FROM SkinsCalcStatus WHERE GameID = ?', [gameId]);
  if (status) return;
  ...
  const context: SkinsHoleContext = { groupId, courseId, gameDate, options };
  ...
  const results = await Promise.allSettled(
    holes.map((holeId) =>
      getSkinsForHole(gameId, holeId, context).catch((error) => {
  ...
  await pool.query(
    'INSERT INTO SkinsCalcStatus (GameID, CalculatedAt) VALUES (?, NOW()) ON DUPLICATE KEY UPDATE CalculatedAt = NOW()',
    [gameId]
  );
}
 ...
 */
export async function getGameSkinsSummary(gameId: number): Promise<GameSkinRow[]> {
  await recalculateAllSkins(gameId);

  const [rows] = await pool.query<any[]>(
    `SELECT sk.HoleID, CONCAT(p.LastName, ', ', p.FirstName) AS name, sk.Score, sk.Validated
     FROM Skins sk
...
/** One hole a player won a skin on, for the Holes Won column — `validated: false` only ever
 * happens when the event's Validate Skins By option is on and this one didn't back up. */
...
export interface SkinsTotals {
  perSkin: number;
  validationMode: SkinsValidationMode;
  rows: SkinsTotalsRow[];
}
 ...
 * or `payout`.
 */
export async function getSkinsTotals(gameId: number): Promise<SkinsTotals> {
  ...
  const options = gameRows.length > 0 ? await getEventOptions(gameRows[0].GroupID) : null;
  ...
  const [rows] = await pool.query<any[]>(
    `SELECT p.PlayerID, p.LastName, p.FirstName, sk.HoleID, sk.Validated, sk.NetScore
     FROM Skins sk
     INNER JOIN Player p ON p.PlayerID = sk.PlayerID
     WHERE sk.GameID = ?
       AND sk.PlayerID NOT IN (SELECT o.PlayerID FROM OptOut o WHERE o.GameID = ?)
     ORDER BY sk.HoleID, p.LastName, p.FirstName`,
    [gameId, gameId]
  );

  const byPlayer = new Map<string, SkinsTotalsRow>();
  for (const r of rows) {
