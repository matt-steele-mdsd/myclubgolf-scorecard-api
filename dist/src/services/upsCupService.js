...
exports.getUpsPointsForGame = getUpsPointsForGame;
...
async function getUpsCupWinners(eventId) {
    const [rows] = await config_1.default.query(`SELECT w.Year, w.PlayerID, CONCAT(p.LastName, ', ', p.FirstName) AS name
     FROM UpsCupWinner w
     INNER JOIN Player p ON p.PlayerID = w.PlayerID
     WHERE w.EventID = ?
     ORDER BY w.Year DESC`, [eventId]);
    return rows.map((r) => ({ year: r.Year, playerId: r.PlayerID, playerName: r.name }));
}
/** Set (or replace) the winner for a given event/year. */
async function setUpsCupWinner(eventId, year, playerId) {
    await config_1.default.query(`INSERT INTO UpsCupWinner (EventID, Year, PlayerID, LastUpdateUser)
     VALUES (?, ?, ?, 'app')
     ON DUPLICATE KEY UPDATE PlayerID = VALUES(PlayerID), LastUpdateDt = CURRENT_TIMESTAMP`, [eventId, year, playerId]);
}
async function deleteUpsCupWinner(eventId, year) {
    await config_1.default.query('DELETE FROM UpsCupWinner WHERE EventID = ? AND Year = ?', [eventId, year]);
}
/**
 ...
 */
async function getQualifyingPlayedCounts(eventId, year) {
    const options = await (0, optionsService_1.getEventOptions)(eventId);
    const minEvents = parseInt(options.ups_minevents, 10) || 0;
    const [todayRows] = await config_1.default.query("SELECT DATE_FORMAT(CURDATE(), '%Y-%m-%d') AS today");
    ...
    for (const date of pastDates) {
        ...
        const [scoreRows] = await config_1.default.query(`SELECT DISTINCT s.PlayerID
       FROM Score s
 ...
 * the first candidate.
 */
async function scorecardTiebreak(gameId, candidatePlayerIds) {
    const [holeRows] = await config_1.default.query('SELECT DISTINCT HoleID FROM Score WHERE GameID = ? ORDER BY HoleID DESC', [gameId]);
    const holesDescending = holeRows.map((r) => r.HoleID);
    const [rows] = await config_1.default.query('SELECT PlayerID, HoleID, NetScore FROM Score WHERE GameID = ? AND PlayerID IN (?)', [gameId, candidatePlayerIds]);
    const netByPlayerHole = new Map();
    for (const r of rows) {
        if (!netByPlayerHole.has(r.PlayerID))
            netByPlayerHole.set(r.PlayerID, new Map());
 ...
 */
async function getMajorWinners(eventId, year) {
    const [majorRows] = await config_1.default.query(`SELECT DATE_FORMAT(EventDate, '%Y-%m-%d') AS date
     FROM EventCalendar
    ...
    let priorWinnersExempt = new Set();
    if (options.ups_includeprioryears) {
        const [winnerRows] = await config_1.default.query('SELECT DISTINCT PlayerID FROM UpsCupWinner WHERE EventID = ? AND Year >= ?', [eventId, year - yearsExemption]);
    ...
    const wonMajorThisYear = new Set();
    const results = [];
    for (const row of majorRows) {
        const [games] = await config_1.default.query('SELECT GameID FROM Game WHERE GroupID = ? AND GameDate = ?', [eventId, row.date]);
        if (games.length === 0) {
            results.push({ date: row.date, playerId: null, playerName: 'TBD', metMinimum: null });
            continue;
        }
        const gameId = games[0].GameID;
        const [totalRows] = await config_1.default.query(`SELECT s.PlayerID, CONCAT(p.LastName, ', ', p.FirstName) AS name, SUM(s.NetScore) AS net
       FROM Score s
       ...
       GROUP BY s.PlayerID`, [gameId, gameId]);
        if (totalRows.length === 0) {
            results.push({ date: row.date, playerId: null, playerName: 'TBD', metMinimum: null });
            continue;
        }
        const totals = totalRows.map((r) => ({ PlayerID: r.PlayerID, name: r.name, net: Number(r.net) }));
        ...
        if (tied.length === 1) {
            ...
            const alreadyQualified = (pid) => wonMajorThisYear.has(pid) || priorWinnersExempt.has(pid);
 ...
 */
async function getUpsPointsForGame(gameId) {
    const [rows] = await config_1.default.query(`SELECT s.PlayerID, CONCAT(p.LastName, ', ', p.FirstName) AS name, SUM(s.NetScore) AS net
     FROM Score s
     INNER JOIN Player p ON p.PlayerID = s.PlayerID
     WHERE s.GameID = ? AND s.PlayerID NOT IN (SELECT o.PlayerID FROM OptOut o WHERE o.GameID = ?)
     GROUP BY s.PlayerID
     ORDER BY net ASC, p.LastName, p.FirstName`, [gameId, gameId]);
    const results = [];
    let prevNet = null;
    let prevPoints = 0;
    rows.forEach((r, i) => {
 ...
 */
async function getStandingsRawData(eventId, year) {
    const options = await (0, optionsService_1.getEventOptions)(eventId);
    ...
    for (const date of qualifyingDates) {
        ...
        for (const p of gamePoints) {
            if (!pointsByPlayer.has(p.playerId))
                pointsByPlayer.set(p.playerId, { name: p.name, points: [] });
            pointsByPlayer.get(p.playerId).points.push(p.points);
        }
    }
    const [paidRows] = await config_1.default.query('SELECT PlayerID FROM UpsCupPaid WHERE EventID = ? AND Year = ?', [eventId, year]);
...
function averageOf(sortedPoints, numScores) {
 ...
 */
async function getCurrentStandings(eventId, year) {
    const { pointsByPlayer, paidSet, numScores, numPlayers } = await getStandingsRawData(eventId, year);
    if (numScores <= 0)
        return [];
    const results = [];
    ...
    if (numPlayers > 0 && paidResults.length > 0) {
        ...
        for (const r of results) {
            if (r.paid)
                r.qualifies = r.average <= cutoffAverage;
        }
    }
    return results;
}
async function getPaidPlayers(eventId, year) {
    const [rows] = await config_1.default.query(`SELECT pp.PlayerID, CONCAT(p.LastName, ', ', p.FirstName) AS name
     FROM UpsCupPaid pp
     INNER JOIN Player p ON p.PlayerID = pp.PlayerID
     WHERE pp.EventID = ? AND pp.Year = ?
     ORDER BY p.LastName, p.FirstName`, [eventId, year]);
    return rows.map((r) => ({ playerId: r.PlayerID, name: r.name }));
}
async function setPlayerPaid(eventId, year, playerId) {
    await config_1.default.query(`INSERT IGNORE INTO UpsCupPaid (EventID, Year, PlayerID, LastUpdateUser) VALUES (?, ?, ?, 'app')`, [eventId, year, playerId]);
}
